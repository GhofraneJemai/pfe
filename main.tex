...
\textbf{M-BERT} & Multilingual BERT \\
...
\section*{Introduction}
\addcontentsline{toc}{section}{Introduction}
Ce chapitre présente les concepts fondamentaux et les technologies clés utilisées dans le développement de Coral.io Expense. Nous y définissons les notions d'intelligence artificielle, d'apprentissage automatique, de traitement du langage naturel, ainsi que les technologies spécifiques telles que l'OCR, les LLM, FAISS et les méthodologies de gestion de projets data science. L'objectif est de fournir un socle théorique commun avant d'aborder les chapitres de réalisation.

\section{Intelligence Artificielle}

\subsection{Définition}
L'Intelligence Artificielle (IA) est une discipline scientifique visant à créer des machines capables de reproduire certaines formes d'intelligence humaine. Elle regroupe l'apprentissage automatique (Machine Learning), l'apprentissage profond (Deep Learning) et le traitement automatique du langage naturel (TAL). L'IA permet d'automatiser des tâches complexes, d'analyser de grandes volumes de données et de prendre des décisions adaptées.
...
Le Deep Learning est une technique d'apprentissage automatique basée sur des réseaux de neurones artificiels comportant plusieurs couches cachées. Il est particulièrement performant pour traiter des données non structurées comme les images, le son ou le texte. Contrairement au Machine Learning traditionnel, le Deep Learning peut apprendre automatiquement des représentations de caractéristiques à partir de données brutes.

\section{Traitement Automatique des Langues (TAL)}

\subsection{Définition}
Le Traitement Automatique des Langues (TAL) est une branche de l'IA qui se concentre sur l'analyse et la compréhension des langues naturelles par les ordinateurs. Il utilise des techniques de traitement du signal, de statistiques et d'apprentissage automatique pour extraire des informations utiles à partir de textes.

\subsection{Tâches principales du TAL}
\begin{itemize}
    \item \textbf{Analyse morphologique} : identification des éléments d'un mot (racine, préfixe, suffixe).
    \item \textbf{Analyse syntaxique} : compréhension de la structure d'une phrase et des relations entre ses éléments.
    \item \textbf{Reconnaissance d'entités nommées} : identification des noms propres (personnes, lieux, organisations).
    \item \textbf{Extraction de relations sémantiques} : découverte des liens entre entités nommées.
    \item \textbf{Analyse de sentiments} : classification de la polarité (positive, négative, neutre) d'un texte.
    \item \textbf{Traduction automatique} : conversion d'un texte d'une langue à une autre.
\end{itemize}
...
\subsubsection{Outils OCR utilisés}
\begin{itemize}
    \item \textbf{PaddleOCR} : moteur OCR léger et multilingue, performant sur les documents français, anglais et arabes. Nécessite une configuration mais offre une très bonne précision.
    \item \textbf{Tesseract} : moteur OCR open source de Google, facile à installer. Moins précis sur les polices atypiques.
    \item \textbf{pdfplumber} : bibliothèque Python pour extraire le texte natif des PDF textuels (sans OCR). Très rapide et précis mais ne fonctionne pas sur les PDF scannés.
\end{itemize}

\subsection{LLM (Large Language Models)}

\subsubsection{Définition}
Un LLM (Modèle de Langage de Grande Taille) est un modèle d'IA entraîné sur des quantités massives de texte. Il est capable de comprendre et de générer du langage naturel. Dans notre projet, les LLM sont utilisés pour structurer des textes OCRisés (extraction de montants, dates, commerçants) et pour comparer sémantiquement un accord de mission avec une note de frais.

\subsubsection{Modèles utilisés}
\begin{itemize}
    \item \textbf{Gemma3:4b} : modèle de 4 milliards de paramètres de Google, spécialisé pour les tâches d'extraction structurée. Rapide (450 ms par requête) et précis sur les documents français. Modèle retenu pour notre solution.
...
Ollama est un serveur local permettant d'exécuter des LLM facilement. Il expose une API REST et gère le téléchargement, l'installation et l'exécution des modèles. Dans notre architecture, Ollama tourne sur le port 11434.
...
FAISS est utilisé pour la détection des doublons de justificatifs. Chaque document est transformé en vecteur (embedding) via SentenceTransformer, puis indexé. Lors de l'upload d'un nouveau document, son embedding est comparé à l'index pour détecter les similarités.

\subsection{SentenceTransformer}

\subsubsection{Définition}
SentenceTransformer est un modèle qui transforme un texte en vecteur (embedding) de dimensions fixes (généralement 384 ou 768). La distance entre deux embeddings mesure la similarité sémantique des textes.

\subsubsection{Modèle utilisé}
Nous utilisons \texttt{all-MiniLM-L6-v2}, un modèle léger (384 dimensions) qui offre un excellent compromis entre précision et rapidité.
...
\subsubsection{Application dans le projet}
Isolation Forest est utilisé pour détecter les montants suspects dans les notes de frais. Il compare un nouveau montant à l'historique des dépenses de l'employé pour une catégorie donnée.

\subsection{Word Embedding}
Le Word Embedding est une technique de représentation des mots sous forme de vecteurs numériques. Les principales techniques sont :
\begin{itemize}
    \item \textbf{One-Hot Encoding} : vecteur binaire de taille égale au nombre de mots dans le vocabulaire.
    \item \textbf{Word2Vec} : développé par Google en 2013, utilise des réseaux de neurones pour apprendre des représentations vectorienses basées sur le contexte.
    \item \textbf{GloVe} : développé en 2014, utilise une approche matricielle basée sur les cooccurrences.
\end{itemize}

\subsection{RNN et LSTM}

\subsubsection{RNN (Récurrent Neural Network)}
Les RNN sont des réseaux de neurones adaptés aux données séquentielles (texte, séries temporelles). Ils prennent en compte les états précédents pour calculer les sorties actuelles. Leur principal inconvénient est la disparition ou l'explosion du gradient pour les séquences longues.

\subsubsection{LSTM (Long Short-Term Memory)}
Les LSTM sont une variante des RNN conçue pour éviter les problèmes de gradient. Ils utilisent des mécanismes de portes (entrée, sortie, oubli) pour contrôler le flux d'informations et mémoriser des informations à long terme.

\subsection{Transformers et BERT}

\subsubsection{Architecture Transformer}
Le Transformer, introduit par Google en 2017, est une architecture de réseau de neurones basée sur un mécanisme d'attention multi-têtes. Contrairement aux RNN, il ne traite pas les séquences de manière linéaire, ce qui le rend plus rapide et plus efficace pour les séquences longues.

\subsubsection{BERT (Bidirectional Encoder Representations from Transformers)}
BERT est un modèle pré-entraîné par Google en 2018. Il utilise une architecture Transformer avec une particularité : il est bidirectionnel, c'est-à-dire qu'il prend en compte le contexte à la fois avant et après chaque mot. BERT existe en deux versions principales :
\begin{itemize}
    \item \textbf{BERT-Base} : 12 couches, 12 têtes d'attention, 110 millions de paramètres.
    \item \textbf{BERT-Large} : 24 couches, 16 têtes d'attention, 340 millions de paramètres.
\end{itemize}

\subsubsection{M-BERT (Multilingual BERT)}
M-BERT est une variante de BERT entraînée sur 104 langues différentes, dont le français et l'arabe. Il est particulièrement adapté aux contextes multilingues.

\section{Méthodologies de gestion de projets Data Science}

\subsection{KDD (Knowledge Discovery in Databases)}
KDD est une méthodologie en cinq étapes pour extraire des connaissances à partir de données :
\begin{enumerate}
    \item \textbf{Sélection} : choix des données pertinentes.
    \item \textbf{Préparation} : nettoyage et transformation.
    \item \textbf{Exploration} : recherche de motifs et tendances.
    \item \textbf{Modélisation} : création de modèles prédictifs.
    \item \textbf{Évaluation} : validation des résultats.
\end{itemize}

\subsection{CRISP-DM (Cross-Industry Standard Process for Data Mining)}
CRISP-DM est la méthodologie de référence pour les projets de Data Science. Elle comprend six phases itératives :
\begin{enumerate}
    \item \textbf{Compréhension du métier} : définition des objectifs et des critères de succès.
    \item \textbf{Compréhension des données} : collecte, exploration et vérification de la qualité des données.
    \item \textbf{Préparation des données} : nettoyage, transformation et intégration.
    \item \textbf{Modélisation} : construction et évaluation des modèles.
    \item \textbf{Évaluation} : vérification que le modèle répond aux besoins métier.
    \item \textbf{Déploiement} : mise en production et intégration dans les processus existants.
\end{itemize}
CRISP-DM a été retenu pour ce projet car il est indépendant de tout outil, itératif et intègre une phase d'évaluation rigoureuse.

\subsection{SEMMA}
SEMMA est une méthodologie développée par SAS en cinq étapes : Sampling (échantillonnage), Exploration (exploration), Modification (modification), Modeling (modélisation), Assessment (évaluation). Contrairement à CRISP-DM, SEMMA ne couvre pas les phases de compréhension du métier et de déploiement.

\section{L'Apprentissage par Transfert (Transfer Learning)}
L'apprentissage par transfert consiste à transférer des connaissances acquises lors de l'apprentissage d'une tâche vers une autre tâche similaire. Dans le TAL, cela se traduit par l'utilisation de modèles pré-entraînés (comme BERT) sur de vastes corpus de texte, puis par leur fine-tuning (ajustement) sur un jeu de données spécifique.

\section{Web Scraping}

\subsection{Définition}
Le web scraping est une technique d'extraction automatisée de données à partir de sites web. Il permet de collecter des données à grande échelle pour alimenter des analyses.

\subsection{Étapes du web scraping}
\begin{enumerate}
    \item Définition des objectifs
    \item Analyse du site web
    \item Sélection des outils (BeautifulSoup, Scrapy, Selenium)
...
\begin{itemize}
    \item \textbf{BeautifulSoup} : bibliothèque Python pour analyser des fichiers HTML et XML.
    \item \textbf{Selenium} : outil d'automatisation de navigateur pour les sites dynamiques.
\end{itemize}

\section{Annotation des données}

\subsection{Annotation manuelle}
L'annotation manuelle est réalisée par des experts humains qui ajoutent des étiquettes aux données. Elle est précise mais coûteuse et chronophage.

\subsection{Annotation automatique}
L'annotation automatique utilise des algorithmes pour étiqueter les données. Elle est rapide et peu coûteuse mais peut produire des erreurs pour les tâches complexes.

\section{Conclusion}
Ce chapitre a présenté les concepts fondamentaux et les technologies utilisées dans le développement de Coral.io Expense. Les notions d'IA, de Machine Learning, de Deep Learning, de TAL, ainsi que les technologies spécifiques comme l'OCR, les LLM, FAISS et SentenceTransformer ont été définies. Les méthodologies CRISP-DM, KDD et SEMMA ont été comparées. Ces bases théoriques permettent d'aborder sereinement les chapitres de réalisation.
% ============================================================
...
\begin{longtable}{|p{2cm}|p{1cm}|p{2cm}|p{8cm}|}
\hline
...
& US-3 & 2 & \begin{itemize}
    ...
    \item L’utilisateur concerné reçoit une notification email.
\end{itemize} \\
\cline{2-4}
& US-4 & 2 & \begin{itemize}
    ...
    \item L’utilisateur reçoit une notification email lors de la désactivation/réactivation.
\end{itemize} \\
\cline{2-4}
...
\captionof{table}{Sprint 1 - User stories et tâches techniques associées}
...
\captionof{table}{Sprint 2 - User stories et tâches techniques associées}
...
\hline
\multirow{12}{*} {\centering Scénario nominal} & 1. L'employé accède à la page « Ajouter une note de frais ». \\
...
\captionof{table}{Sprint 3 - User stories et tâches techniques associées}
...
\hline
\endlastfoot
\textbf{Titre} & Téléverser un justificatif et déclencher les analyses automatiques \\
\hline
\textbf{Acteur} & Employé \\
\hline
\textbf{But} & Déposer un justificatif (facture ou accord) lors de la création d'une note de frais et déclencher l'ensemble des contrôles automatiques (doublons, OCR, remplissage, montants suspects, comparaison avec accord de mission) \\
\hline
\textbf{Précondition} & L'employé est authentifié et est en train de créer/modifier une note de frais \\
\hline
\multirow{12}{*}{\textbf{Scénario nominal}} & 1. L'employé clique sur "Téléverser un justificatif" \\
...
\hline
\multirow{6}{*}{\textbf{Scénario alternatif}} & 4.1 Format invalide : le système affiche "Format non supporté" \\
...
\hline
\textbf{Post-condition} & Le justificatif est stocké sur le serveur, associé à la note de frais, et l'ensemble des résultats d'analyse (doublons, anomalies de montant, comparaison accord) est enregistré dans la base de données \\
\hline
\end{longtable}

\subsection*{Description textuelle -- Gérer les anomalies IA}
\begin{longtable}{|p{3cm}|p{11cm}|}
\hline
\textbf{Élément} & \textbf{Description} \\
\hline
...
\hline
\endlastfoot
\textbf{Titre} & Gérer les anomalies IA \\
\hline
\textbf{Acteur} & Manager / Administrateur \\
\hline
\textbf{But} & Visualiser et traiter les anomalies détectées automatiquement par l'IA sur les notes de frais\\
\hline
\textbf{Précondition} & L'utilisateur est authentifié avec le rôle Manager ou Administrateur \\
\hline
\multirow{8}{*}{\textbf{Scénario nominal}} & 1. L'utilisateur accède à son tableau de bord (ManagerDashboard ou AdminDashboard) \\
...
\hline
\textbf{Scénario alternatif} & Aucune anomalie : la section affiche "Aucune anomalie à signaler" \\
\hline
\textbf{Post-condition} & L'utilisateur a pris une décision sur les notes anormales ; la note de frais avance dans son workflow de validation avec le statut approprié \\
\hline
\end{longtable}
...
\centering
\fbox{\includegraphics[width=0.8\textwidth]{logo_hash.png}}
...
\begin{minipage}{0.3\textwidth}
\centering
\fbox{\includegraphics[width=0.8\textwidth]{logo_bert.png}}
...
\begin{minipage}{0.3\textwidth}
\centering
\fbox{\includegraphics[width=0.8\textwidth]{logo_faiss.png}}
...
\bibitem{rydoo} Rydoo, \textit{Solution de gestion des notes de frais Rydoo}, [En ligne], Disponible : \url{https://www.rydoo.com/}, (Consulté le 15/02/2025).
...
\bibitem{crispdm} Chapman, P., Clinton, J., Kerber, R., Khabaza, T., Reinartz, T., Shearer, C., Wirth, R., \textit{CRISP-DM 1.0: Step-by-step data mining guide}, SPSS Inc., 2000.
\bibitem{kdd} Fayyad, U., Piatetsky-Shapiro, G., Smyth, P., \textit{From Data Mining to Knowledge Discovery in Databases}, AI Magazine, Vol. 17(3), pp. 37-54, 1996.
\bibitem{semma} SAS Institute, \textit{SEMMA: Sample, Explore, Modify, Model, Assess}, [En ligne], Disponible : \url{https://www.sas.com/}, (Consulté le 25/05/2025).

% Intelligence Artificielle et Machine Learning
\bibitem{mitchell1997} Mitchell, T. M., \textit{Machine Learning}, McGraw-Hill, 1997.
\bibitem{goodfellow2016} Goodfellow, I., Bengio, Y., Courville, A., \textit{Deep Learning}, MIT Press, 2016.

% Traitement Automatique des Langues
\bibitem{jurafsky2009} Jurafsky, D., Martin, J. H., \textit{Speech and Language Processing}, Pearson Prentice Hall, 2009.

% Word Embedding
\bibitem{mikolov2013} Mikolov, T., Chen, K., Corrado, G., Dean, J., \textit{Efficient Estimation of Word Representations in Vector Space}, arXiv:1301.3781, 2013.
\bibitem{pennington2014} Pennington, J., Socher, R., Manning, C. D., \textit{GloVe: Global Vectors for Word Representation}, Proceedings of EMNLP, 2014.

% RNN et LSTM
\bibitem{hochreiter1997} Hochreiter, S., Schmidhuber, J., \textit{Long Short-Term Memory}, Neural Computation, Vol. 9(8), pp. 1735-1780, 1997.

% Transformers et BERT
\bibitem{vaswani2017} Vaswani, A., et al., \textit{Attention Is All You Need}, arXiv:1706.03762, 2017.
\bibitem{devlin2018} Devlin, J., Chang, M. W., Lee, K., Toutanova, K., \textit{BERT: Pre-training of Deep Bidirectional Transformers for Language Understanding}, arXiv:1810.04805, 2018.

% OCR
\bibitem{paddleocr} PaddlePaddle, \textit{PaddleOCR: Awesome OCR Toolkits based on PaddlePaddle}, [En ligne], Disponible : \url{https://github.com/PaddlePaddle/PaddleOCR}, (Consulté le 15/05/2025).
\bibitem{tesseract} Google, \textit{Tesseract OCR Documentation}, [En ligne], Disponible : \url{https://github.com/tesseract-ocr/tesseract}, (Consulté le 15/05/2025).

% FAISS et SentenceTransformer
\bibitem{faiss} Johnson, J., Douze, M., Jégou, H., \textit{Billion-scale similarity search with GPUs}, IEEE Transactions on Big Data, Vol. 7(3), pp. 535-547, 2019.
\bibitem{sentencebert} Reimers, N., Gurevych, I., \textit{Sentence-BERT: Sentence Embeddings using Siamese BERT-Networks}, arXiv:1908.10084, 2019.

% Isolation Forest
\bibitem{liu2008} Liu, F. T., Ting, K. M., Zhou, Z. H., \textit{Isolation Forest}, Proceedings of the 8th IEEE International Conference on Data Mining, pp. 413-422, 2008.

% LLM
\bibitem{gemma} Google, \textit{Gemma: Open Models Based on Gemini Research and Technology}, [En ligne], Disponible : \url{https://ai.google.dev/gemma}, (Consulté le 20/05/2025).
\bibitem{mistral} Mistral AI, \textit{Mistral 7B Documentation}, [En ligne], Disponible : \url{https://mistral.ai/news/announcing-mistral-7b/}, (Consulté le 20/05/2025).
\bibitem{llama} Meta, \textit{Llama 3: Meta's Latest Large Language Model}, [En ligne], Disponible : \url{https://llama.meta.com/}, (Consulté le 20/05/2025).
\bibitem{ollama} Ollama, \textit{Ollama Documentation}, [En ligne], Disponible : \url{https://ollama.com/}, (Consulté le 20/05/2025).

% Transfer Learning
\bibitem{pan2010} Pan, S. J., Yang, Q., \textit{A Survey on Transfer Learning}, IEEE Transactions on Knowledge and Data Engineering, Vol. 22(10), pp. 1345-1359, 2010.

% Web Scraping
\bibitem{beautifulsoup} Richardson, L., \textit{Beautiful Soup Documentation}, [En ligne], Disponible : \url{https://www.crummy.com/software/BeautifulSoup/}, (Consulté le 25/05/2025).
