...
\usepackage{pdfpages}
\usepackage{titlesec}
\setlist[itemize]{leftmargin=3.5cm}

% ============================================================
% INDENTATION DES TITRES DANS LE CORPS DU DOCUMENT
% ============================================================

% Pour les sections (1, 2...)
\titleformat{\section}
  {\normalfont\Large\bfseries}
  {\thesection.}%  ← POINT ajouté ici
  {1em}
  {}
  [\vspace{0.3\baselineskip}]

\titlespacing*{\section}
  {0em}      % Pas d'indentation pour section
  {0.5\baselineskip}
  {0pt}

% Pour les sous-sections (1.1, 1.2...)
\titleformat{\subsection}
  {\normalfont\large\bfseries}
  {\thesubsection.}%  ← POINT ajouté ici
  {1em}
  {}
  [\vspace{0.3\baselineskip}]
...
% Pour les sous-sous-sections (1.1.1...)
\titleformat{\subsubsection}
  {\normalfont\bfseries}
  {\thesubsubsection.}%  ← POINT ajouté ici
  {1em}
  {}
  [\vspace{0.3\baselineskip}]
...
% Pour les subsubsection NON numerotes (a), b), c)...)
...
% ============================================================
% CRÉATION D'UN NOUVEAU NIVEAU a), b), c)...
% QUI S'ALIGNE CORRECTEMENT SOUS \subsubsection
% ============================================================

% Nouvelle définition pour avoir a), b), c)...
\newcounter{subsubsubcounter}[subsubsection]
\renewcommand{\thesubsubsubcounter}{\alph{subsubsubcounter}}

\makeatletter
\newcommand{\subsubsubsection}[1]{%
  \stepcounter{subsubsubcounter}%
  \vspace{0.2\baselineskip}%
  \noindent
  \hspace*{8em}\textbf{\thesubsubsubcounter) #1}%  ← PAS DE POINT supplémentaire
  \par
  \vspace{0.1\baselineskip}%
}
\makeatother

% Nouvelle commande avec étoile SANS numéro
\newcommand{\subsubsubsubsectionstar}[1]{%
  \vspace{0.2\baselineskip}%
  \noindent
  \hspace*{9em}{\Large$\star$} \textbf{#1}%
  \par
  \vspace{0.1\baselineskip}%
}

% ============================================================
% CRÉATION D'UN VRAI NIVEAU 1.1.1.1 (QUATRE CHIFFRES)
% ============================================================

\newcounter{subsubsubsub}[subsubsection]
\renewcommand{\thesubsubsubsub}{\thesubsubsection.\arabic{subsubsubsub}}

\makeatletter
\newcommand{\subsubsubsub}[1]{%
  \refstepcounter{subsubsubsub}%
  \vspace{0.2\baselineskip}%
  \noindent
  \hspace*{6em}\textbf{\thesubsubsubsub. #1}%  ← POINT et ESPACE
  \par
  \vspace{0.1\baselineskip}%
}
\makeatother

% Pour inclure ce niveau dans la numérotation
\setcounter{secnumdepth}{4}
\setcounter{tocdepth}{4}

% ============================================================
% CORRECTION NUMEROTATION : Section = 1, Sous-section = 1.1
% ============================================================

\makeatletter
% Section : 5 (sans point)
\renewcommand{\thesection}{\arabic{section}}

% Subsection : 5.1 (sans point final)
\renewcommand{\thesubsection}{\thesection.\arabic{subsection}}

% Subsubsection : 5.1.1 (sans point final)
\renewcommand{\thesubsubsection}{\thesubsection.\arabic{subsubsection}}

% Subsubsubsub : 5.1.1.1 (sans point final, sera ajouté dans l'affichage)
\renewcommand{\thesubsubsubsub}{\thesubsubsection.\arabic{subsubsubsub}}

% Réinitialiser section à chaque chapitre
\@addtoreset{section}{chapter}

% Forcer le compteur section à 0 au début de chaque chapitre
\let\oldchapter\chapter
\def\chapter{\clearpage\setcounter{section}{0}\oldchapter}
\makeatother
...
% Pour inclure subsubsection dans la numérotation et la TOC
...
% Configuration qui fonctionne (sans gras)
\renewcommand{\cftchapfont}{\normalfont\normalsize}
\renewcommand{\cftchappagefont}{\normalfont\normalsize}

% Remettre le mot "Chapitre" devant chaque entrée
...
% === PAGE 1 : TON PDF ===
\includepdf[pages=1]{Remerciement.pdf}
\includepdf[pages=1]{Dédicaces_Manel.pdf}
\includepdf[pages=1]{Dédicaces_Ghofrane.pdf}
...
\textbf{R\&D} & Recherche et Développement \\
\textbf{KPI} & Key Performance Indicator \\
\textbf{TVA} & Taxe sur la Valeur Ajoutée \\
\textbf{FAQ} & Frequently Asked Questions \\
\end{longtable}
\clearpage
...
\subsubsection{Positionnement client}\\
...
\subsubsection{Valeurs et organisation}\\
...
\begin{itemize}
 ...
 \includegraphics[width=\textwidth, height=0.3\textheight]{sap.png}
 ...
 \includegraphics[width=\textwidth,height=0.3\textheight]{rydoo.jpg}
...
Les solutions actuelles présentent des forces et des faiblesses. Pour éclairer notre choix, nous les comparons selon les critères suivants. Chaque critère est d’abord défini brièvement.
...
Choisir une méthodologie de développement adaptée est déterminant pour la réussite d'un projet logiciel. Ce choix dépend de plusieurs facteurs : la nature du projet, la flexibilité attendue, la complexité des fonctionnalités, la taille de l'équipe et les contraintes de délais.

Notre projet ayant une double nature — développement applicatif classique et intégration de composants d'intelligence artificielle — nous avons fait le choix d'adopter deux méthodologies complémentaires :
\begin{itemize}
 \item \textbf{Scrum} : pour la gestion de projet globale, l'organisation des sprints et la coordination de l'équipe.
 \item \textbf{CRISP-DM} : pour piloter spécifiquement le développement des composants d'intelligence artificielle (détection d'anomalies, OCR, LLM).
\end{itemize}
...
\subsubsection{Présentation de la méthodologie}

CRISP-DM (Cross-Industry Standard Process for Data Mining) est une méthodologie de référence en six phases itératives pour les projets de data mining et de science des données. Développée en 1996 par un consortium d'entreprises (NCR, SPSS, DaimlerChrysler), elle est devenue un standard de fait dans l'industrie. Contrairement à d'autres approches comme KDD (Knowledge Discovery in Databases) ou SEMMA (Sample, Explore, Modify, Model, Assess), CRISP-DM présente plusieurs avantages : elle est indépendante de tout outil ou technologie, elle intègre une phase de déploiement rigoureuse, et ses phases itératives permettent une amélioration continue des modèles.

Les six phases de CRISP-DM sont les suivantes :

\begin{figure}[H]
 \centering
 \includegraphics[width=0.7\textwidth]{Diagramme_du_Processus_CRISP-DM.png}
 \caption{Les six phases de CRISP-DM}
 \label{fig:crispdm_cycle}
\end{figure}

\begin{enumerate}
 \item \textbf{Compréhension du métier} : Définition des objectifs, des contraintes et des critères de succès du projet. Cette phase permet de traduire le besoin métier en problème data science.
 
 \item \textbf{Compréhension des données} : Collecte, description et exploration des données disponibles. Elle permet d'identifier la qualité, la quantité et la pertinence des données par rapport au problème posé.
 
 \item \textbf{Préparation des données} : Nettoyage, transformation et construction des jeux de données finaux. Cette phase est souvent la plus chronophage mais essentielle pour la qualité des modèles.
 
 \item \textbf{Modélisation} : Sélection et application des techniques de modélisation, paramétrage et évaluation. Plusieurs techniques peuvent être testées et comparées.
 
 \item \textbf{Évaluation} : Vérification que le modèle répond aux objectifs métier avant déploiement. Cette phase valide la performance du modèle sur des données de test.
 
 \item \textbf{Déploiement} : Mise en production du modèle, planification du monitoring et de la maintenance. Cette phase assure que le modèle est exploitable par les utilisateurs finaux.
\end{enumerate}

\subsubsection{Pourquoi CRISP-DM ?}

CRISP-DM n'a pas été choisi par simple effet de mode, mais parce qu'il répond précisément aux caractéristiques des composants d'intelligence artificielle de notre projet :

\begin{itemize}
 \item \textbf{Une approche structurée indispensable} : La détection des anomalies dans les notes de frais ne se limite pas à l'implémentation d'un modèle. Elle nécessite une approche couvrant l'ensemble du cycle de vie des données, de la compréhension du métier au déploiement en production.
 
 \item \textbf{Une indépendance technologique} : CRISP-DM est indépendant de tout outil ou technologie, ce qui permet de l'adapter parfaitement à notre architecture microservices (Python/FastAPI, Java/Spring Boot, PostgreSQL).
 
 \item \textbf{Des phases itératives pour l'amélioration continue} : Ses six phases correspondent parfaitement au besoin d'amélioration continue des modèles (ajustement des seuils, réentraînement, etc.).
 
 \item \textbf{Une collaboration renforcée métier-technique} : Il favorise une collaboration étroite entre experts métier (administrateurs, managers) et data scientists, essentielle pour définir ce qu'est une « anomalie » dans le contexte tunisien.
 
 \item \textbf{Une phase d'évaluation rigoureuse} : Il intègre une phase d'évaluation (matrice de confusion, métriques de précision/rappel, F1-score) avant le déploiement, limitant ainsi les risques de faux positifs en production.
 
 \item \textbf{Un déploiement industriel maîtrisé} : Contrairement à SEMMA qui s'arrête à l'évaluation, CRISP-DM intègre une phase de déploiement rigoureuse, essentielle pour mettre en production les modèles FAISS et Isolation Forest.
\end{itemize}
...
\subsubsection{ Versionnement et collaboration}
...
\subsubsection{ Design et maquettage}
...
\subsubsection{ Frameworks et langages front-end}
...
\subsubsection{ Frameworks et langages back-end}
...
\subsubsection{Tests et documentation d'API}
...
Ce chapitre présente les concepts fondamentaux et les technologies clés utilisées dans le développement de \textbf{Coral.io Expense}. Plutôt qu’une simple liste de définitions, nous détaillons ici les principes scientifiques et techniques qui guident nos choix architecturaux. Nous introduisons d’abord les notions d’intelligence artificielle et d’apprentissage automatique, puis nous analysons en profondeur chaque technologie implémentée dans notre solution.

\section{Intelligence Artificielle}

\subsection{Définition et principes}

L’intelligence artificielle (IA) est une discipline scientifique visant à créer des machines capables de reproduire certaines formes d’intelligence humaine. Elle désigne un ensemble de théories et de techniques visant à doter les machines de capacités cognitives proches de celles de l’humain (perception, raisonnement, apprentissage, décision). L’IA regroupe l’apprentissage automatique (Machine Learning) et l’apprentissage profond (Deep Learning). Elle permet d’automatiser des tâches complexes, d’analyser de grands volumes de données et de prendre des décisions adaptées.

\begin{figure}[H]
 \centering
 \includegraphics[width=0.7\textwidth]{ia_principes_fondamentaux.png}
 \caption{Les quatre piliers de l’IA moderne. Notre projet utilise principalement l’apprentissage et l’interaction (via extraction de données et chatbot).}
 \label{fig:ia-principes}
\end{figure}

L’IA repose sur plusieurs principes :
\begin{itemize}
 \item \textbf{Représentation des connaissances} : comment encoder l’information (ex. : texte, images, vecteurs).
 \item \textbf{Raisonnement} : déduire de nouvelles informations à partir de faits connus.
 \item \textbf{Apprentissage} : ajuster automatiquement les paramètres d’un modèle à partir de données.
 \item \textbf{Interaction} : réagir à des entrées extérieures (requêtes utilisateur, documents).
\end{itemize}
Dans notre projet, nous combinons apprentissage automatique et raisonnement sur des extraits textuels.

\subsection{Apprentissage automatique (Machine Learning)}

Le Machine Learning (ML) est un sous-domaine de l’IA où un modèle apprend des relations statistiques à partir de données, sans être programmé explicitement pour chaque tâche.

\paragraph{Principe général :}
Soit $X$ les données d’entrée et $Y$ la sortie souhaitée. Un algorithme de ML cherche une fonction $f$ telle que $f(X) \approx Y$, en minimisant une erreur sur des exemples.
...
\begin{itemize}
 ...
 \item \textbf{Apprentissage non supervisé} : le modèle cherche des structures dans des données non étiquetées.
...
\begin{figure}[H]
 \centering
 \includegraphics[width=0.8\textwidth]{ml_trois_paradigmes.jpg}
 \caption{Classification des approches ML}
 \label{fig:ml-paradigms}
\end{figure}

\subsection{Apprentissage profond (Deep Learning)}

Le Deep Learning est une technique d’apprentissage automatique basée sur des réseaux de neurones artificiels comportant plusieurs couches cachées. Chaque couche transforme les données en représentations de plus en plus abstraites. Il est particulièrement performant pour traiter des données non structurées comme les images, le son ou le texte.

\paragraph{Principe :}
Un neurone reçoit des entrées pondérées, applique une fonction d’activation non linéaire et transmet le résultat. Par empilement de couches, le réseau apprend des hiérarchies de caractéristiques.

Les modèles Transformer (utilisés dans les LLM) généralisent ce principe avec des mécanismes d’\textit{attention}, permettant de capturer des dépendances à longue distance dans le texte.
...
L'OCR (Reconnaissance Optique de Caractères) est une technologie permettant de convertir des images de texte en texte exploitable par une machine. Elle est essentielle pour traiter les justificatifs de frais (factures, accords de mission) déposés sous forme de PDF scannés ou d'images.

\paragraph{Principe opérationnel :}
\begin{enumerate}
 \item \textbf{Prétraitement} : binarisation, redressement, suppression du bruit.
 \item \textbf{Détection des zones de texte} (layout analysis).
 \item \textbf{Reconnaissance de caractères} : réseau de neurones ou correspondance de motifs.
 \item \textbf{Post-traitement} : correction orthographique, mise en page.
\end{enumerate}

\begin{figure}[H]
 \centering
 \includegraphics[width=0.85\textwidth]{ocr_chain_steps.png}
 \caption{Étapes séquentielles de l’OCR.}
 \label{fig:ocr-chain}
\end{figure}
...
\begin{itemize}
 \item \textbf{PaddleOCR} : moteur OCR léger et multilingue, basé sur un détecteur de texte (DBnet) et un reconnaisseur CRNN. Performant sur les documents français, anglais et arabes. Il est notre outil principal pour les images et les PDF scannés.
 \item \textbf{Tesseract} : moteur OCR open source de Google utilisant LSTM. Solution de repli lorsque PaddleOCR échoue (taux de confiance < 0,7).
 \item \textbf{pdfplumber} : bibliothèque Python pour extraire le texte natif des PDF textuels. Très rapide (100x plus rapide qu’un OCR), elle est privilégiée pour les accords de mission au format PDF texte. Elle ne fait pas d’OCR, mais lit les flux texte intégrés.
\end{itemize}

\subsection{LLM (Large Language Models)}

\subsubsection{Définition et principe fondamental}

Un LLM (Modèle de Langage de Grande Taille) est un modèle d'IA (réseau de neurones de type Transformer) entraîné à prédire le prochain mot dans une séquence de texte, sur des milliards de tokens. Il est capable de comprendre et de générer du langage naturel. Dans notre projet, les LLM sont utilisés pour deux tâches principales : la structuration de textes OCRisés et la comparaison sémantique entre un accord de mission et une note de frais.

\begin{figure}[H]
 \centering
 \includegraphics[width=0.7\textwidth]{llm_transformer_attention.png}
 \caption{Principe général d’un Large Language Model (LLM).}
 \label{fig:llm_schema}
\end{figure}

\paragraph{Propriétés clés :}
\begin{itemize}
 \item \textbf{Compréhension contextuelle} : chaque mot est représenté en fonction des mots environnants.
 \item \textbf{Génération} : production de texte cohérent via échantillonnage probabiliste.
 \item \textbf{Apprentissage zéro-shot / few-shot} : capacité à répondre à des tâches non vues lors de l’entraînement.
\end{itemize}

\subsubsection{Modèles évalués et modèle retenu}

Nous avons comparé trois modèles sur trois critères : temps de réponse (ms), exactitude de l’extraction structurée (F1-score sur champs : date, montant TTC, fournisseur), et consommation mémoire.

\begin{itemize}
 \item \textbf{Gemma3:4b} : modèle de 4 milliards de paramètres de Google, spécialisé pour l'extraction structurée. Rapide (450 ms) et précis (F1=0,92) sur documents français. \textbf{Modèle retenu.}
 \item \textbf{Mistral 7B} : 7 milliards de paramètres, optimisé pour le suivi d'instructions. Bonnes performances générales mais plus lent (850 ms, F1=0,93).
 \item \textbf{Llama 3 8B} : 8 milliards de paramètres de Meta, très performant en compréhension naturelle. Plus lent (1200 ms, F1=0,94).
\end{itemize}

\subsubsection{Serveur Ollama}

Ollama est un serveur local qui encapsule les LLM dans un processus, les maintenant en mémoire entre les requêtes pour éviter des chargements répétés. Il expose une API REST conforme à OpenAI, gérant le téléchargement, l’installation et l’exécution des modèles. Dans notre architecture, Ollama tourne sur le port 11434 et dessert à la fois l’extraction de factures, l’extraction structurée des accords de mission, la comparaison sémantique et le chatbot hybride.

\subsection{FAISS (Facebook AI Similarity Search)}

\subsubsection{Définition et principe de recherche vectorielle}
...
FAISS implémente des algorithmes de recherche des plus proches voisins (similarité cosinus, distance euclidienne) dans un espace vectoriel de grande dimension.  

\paragraph{Principe d’indexation :}
\begin{itemize}
 \item Un index plat (FlatL2) compare la requête à tous les vecteurs – exact mais lent.
 \item Un index IVF (Inverted File) partitionne l’espace en cellules ; seule la cellule pertinente est parcourue.
\end{itemize}
Nous utilisons un index IVF avec 128 centroïdes, pour un temps de réponse < 10 ms pour 10 000 documents.

\subsubsection{Application dans le projet : détection de doublons}

Chaque justificatif est transformé en embedding via SentenceTransformer, puis indexé dans FAISS. Lors de l’upload d’un nouveau document, son embedding est comparé à l’index pour détecter les similarités. Si la similarité cosinus dépasse le seuil de 0,95 (fixé après optimisation), un doublon est signalé.

\subsection{SentenceTransformer}

\subsubsection{Définition et principe des embeddings}

Un embedding est une représentation vectorielle d’un texte (mot, phrase ou document) dans un espace continu de dimensions fixes (généralement 384 ou 768). Deux textes sémantiquement proches auront des vecteurs proches, tandis que des textes de sens différent auront des vecteurs éloignés. Cette propriété permet de mesurer la similarité sémantique entre deux documents par le calcul de la distance (cosinus, euclidienne, etc.) entre leurs embeddings.

SentenceTransformer est un modèle qui transforme un texte en embedding. L’apprentissage se fait par triplet : le modèle apprend à rapprocher les paires similaires et éloigner les paires dissemblables. Dans notre projet, les embeddings sont utilisés pour la détection des doublons.

\subsubsection{Modèle utilisé}

Nous utilisons \texttt{all-MiniLM-L6-v2}, une distillation de BERT (22 millions de paramètres, sortie 384 dimensions). Il offre un excellent compromis entre précision (98\%) et rapidité (120 ms par fichier).

\subsection{Isolation Forest}

\subsubsection{Définition et principe mathématique}

Isolation Forest est un algorithme non supervisé de détection d'anomalies. Il isole les anomalies non pas par modélisation des données normales, mais par une partition aléatoire de l’espace. Une observation anormale est plus facile à séparer (nécessite moins de coupures) qu’une observation normale.

\paragraph{Algorithme :}
\begin{enumerate}
 \item Construire $T$ arbres de décision aléatoires.
 \item Pour chaque point, calculer la profondeur moyenne d’isolement $E[h(x)]$ (nombre de coupures nécessaires pour séparer le point).
 \item Un point est d’autant plus anormal que sa profondeur moyenne est faible.
\end{enumerate}
Le score d’anomalie est défini par $s(x,n) = 2^{-\frac{E[h(x)]}{c(n)}}$, où $c(n)$ est un facteur de normalisation.
...
\subsubsection{Définition et principe d’un microservice IA}
...
Un microservice est un processus indépendant, exposant une API sur un port, dédié à une fonction métier.

\subsubsection{Architecture déployée dans Coral.io Expense}

L’intelligence artificielle n’est pas intégrée directement dans le backend principal. Nous avons opté pour une architecture en microservices découplés :

\begin{itemize}
 \item \textbf{Backend Java (Spring Boot, port 8888)} : orchestration des appels aux services IA. Il reçoit les requêtes du front-end Angular et invoque les microservices Python appropriés.
...
\begin{itemize}
 \item D’isoler la logique d’IA, de faciliter son évolution et de la scaler indépendamment du reste de l’application.
 \item De scaler l’OCR/LLM sans toucher au backend principal.
 \item De changer de modèle de LLM sans recompiler Java.
 \item D’isoler les pannes (un crash de l’OCR ne bloque pas toute l’application).
\end{itemize}
...
L’apprentissage par transfert consiste à réutiliser des connaissances acquises lors de l’apprentissage d’une tâche source pour une tâche cible différente mais apparentée. On distingue deux approches :
\begin{itemize}
 \item \textbf{Feature extraction} : les couches pré-entraînées sont figées, on n’entraîne qu’un classifieur final.
 \item \textbf{Fine-tuning} : on ajuste partiellement les poids du modèle sur la nouvelle tâche.
\end{itemize}

Dans notre projet, nous utilisons cette approche à plusieurs niveaux :

\begin{itemize}
 \item \textbf{SentenceTransformer all-MiniLM-L6-v2} : utilisé en extraction de caractéristiques (sans fine-tuning) pour la détection de doublons. La similarité sémantique générale est déjà bien capturée et le budget limité en données ne justifie pas un réentraînement.
 \item \textbf{LLM Gemma3:4b} : utilisé en prompting (few-shot) sans fine-tuning. Les LLM modernes excellent en extraction zéro-shot.
 \item \textbf{Isolation Forest} : entraîné \textit{ab initio} car les distributions de montants dépendent entièrement du contexte propre à l’entreprise.
\end{itemize}

\begin{figure}[H]
 \centering
 \includegraphics[width=0.8\textwidth]{transfer_learning.png}
 \caption{L’apprentissage par transfert}
...
Ce chapitre a détaillé les principes scientifiques et techniques sous-jacents aux composants IA de Coral.io Expense. Nous avons expliqué le fonctionnement et les interactions de l’OCR, des LLM, de FAISS, de SentenceTransformer, d’Isolation Forest et de l’architecture microservices. L’accent a été mis sur les principes fondamentaux (représentation vectorielle, isolation d’anomalies, mécanismes d’attention dans les Transformers, apprentissage par transfert) plutôt que sur de simples définitions. Ces bases théoriques permettent d’aborder sereinement les chapitres de réalisation où chaque brique est implémentée et validée.
...
Dans ce chapitre, nous présentons les besoins du projet d'application de gestion de notes de frais Coral.io Expense. Nous identifierons les différentes parties prenantes, définirons les besoins fonctionnels et non fonctionnels, en mettant un accent particulier sur les fonctionnalités d'intelligence artificielle, de conversion de devises et d'expérience utilisateur avancée.
...
\multirow{5}{*}{Détection des anomalies} & 64 & En tant qu'administrateur et manager, je veux que le système détecte automatiquement les doublons de justificatifs (factures et accords) lors de l'upload afin d'éviter les soumissions frauduleuses ou en erreur. & 5 & élevée \\
...
Le tableau ci-dessous présente les critères d'acceptation associés aux user stories prioritaires identifiées pour Coral.io Expense. Ces critères définissent les conditions précises que chaque fonctionnalité doit satisfaire pour être considérée comme complète et validée par le client.

\captionof{table}{Critères d’acceptation des User Stories prioritaires}
\label{tab:Critères}
\begin{longtable}{|p{2.5cm}|p{3cm}|p{1.5cm}|p{8cm}|}
\hline
\textbf{Fonctionnalité} & \textbf{User Story / Tâche} & \textbf{Est.} & \textbf{Critères d’acceptation} \\
\hline
\endfirsthead
\hline
\textbf{Fonctionnalité} & \textbf{User Story / Tâche} & \textbf{Est.} & \textbf{Critères d’acceptation} \\
...
\multirow{5}{*}{Comptes} & US-1 :  Création compte & 2 & \begin{itemize}
...
\end{itemize} \\
\cline{2-4}
& US-2 : Consultation liste comptes & 2 & \begin{itemize}
...
\end{itemize} \\
\cline{2-4}
& US-3 : Modification compte & 2 & \begin{itemize}
...
\multirow{3}{*}{Départements} & US-11 : Création département & 2 & \begin{itemize}
...
\end{itemize} \\
\cline{2-4}
& US-12 : Consultation liste départements & 2 & \begin{itemize}
...
\end{itemize} \\
\cline{2-4}
& US-16 : Activation/Désactivation département & 2 & \begin{itemize}
...
\multirow{4}{*}{Projets} & US-17 : Création projet & 2 & \begin{itemize}
...
\end{itemize} \\
\cline{2-4}
& US-18 : Consultation liste projets & 2 & \begin{itemize}
...
\end{itemize} \\
\cline{2-4}
& US-20 : Clôture projet & 2 & \begin{itemize}
...
\end{itemize} \\
\cline{2-4}
& US-23 : Affectation employé à projet & 2 & \begin{itemize}
...
\multirow{4}{*}{Catégories} & US-25 : Création catégorie & 3 & \begin{itemize}
...
\end{itemize} \\
\cline{2-4}
& US-26 : Consultation liste catégories & 2 & \begin{itemize}
 \item L’administrateur voit la liste des catégories avec plafonds, statut et champs dynamiques.
 \item Possibilité de filtrer par statut (actif/inactif).
\end{itemize} \\
\cline{2-4}
& US-29 : Activation/Désactivation catégorie & 2 & \begin{itemize}
...
\end{itemize} \\
\cline{2-4}
& US-30 : Champs personnalisés dynamiques & 8 & \begin{itemize}
...
\multirow{11}{*}{Notes de frais} & US-32 : Création note de frais & 8 & \begin{itemize}
...
\end{itemize} \\
\cline{2-4}
& US-33 : Gestion des devises & 5 & \begin{itemize}
...
\end{itemize} \\
\cline{2-4}
& US-34 : Consultation liste notes & 5 & \begin{itemize}
...
\end{itemize} \\
\cline{2-4}
& US-35 : Modification/Suppression note & 5 & \begin{itemize}
...
\end{itemize} \\
\cline{2-4}
& US-36 : Notification employé & 3 & \begin{itemize}
...
\end{itemize} \\
\cline{2-4}
& US-37 : Consultation notes manager & 5 & \begin{itemize}
...
\end{itemize} \\
\cline{2-4}
& US-38 : Notification manager & 3 & \begin{itemize}
...
\end{itemize} \\
\cline{2-4}
& US-40 : Validation note & 3 & \begin{itemize}
...
\end{itemize} \\
\cline{2-4}
& US-41 : Refus note & 3 & \begin{itemize}
...
\end{itemize} \\
\cline{2-4}
& US-42 : Impact budgétaire prévisionnel & 3 & \begin{itemize}
...
\end{itemize} \\
\cline{2-4}
& US-43 : Consommation employé par projet & 5 & \begin{itemize}
...
\multirow{10}{*}{Remboursements} & US-49 : Alertes événements critiques & 4 & \begin{itemize}
...
\end{itemize} \\
\cline{2-4}
& US-50 : Notes internes confidentielles & 3 & \begin{itemize}
...
\end{itemize} \\
\cline{2-4}
& US-51 : Consultation notes validées & 5 & \begin{itemize}
...
\end{itemize} \\
\cline{2-4}
& US-52 : Remboursement note & 5 & \begin{itemize}
...
\end{itemize} \\
\cline{2-4}
& US-54 : Consultation ordre de paiement & 5 & \begin{itemize}
...
\end{itemize} \\
\cline{2-4}
& US-55 : Ordres de paiement en attente & 3 & \begin{itemize}
...
\end{itemize} \\
\cline{2-4}
& US-56 : Confirmation paiement & 5 & \begin{itemize}
...
\end{itemize} \\
\cline{2-4}
& US-58 : Budget restant avant remboursement & 3 & \begin{itemize}
...
\end{itemize} \\
\cline{2-4}
& US-59 : Dashboard budgets projets & 3 & \begin{itemize}
...
\end{itemize} \\
\cline{2-4}
& US-62 : Export Excel & 3 & \begin{itemize}
...
\multirow{4}{*}{Anomalies} & US-64 : Détection doublons justificatifs & 5 & \begin{itemize}
...
\end{itemize} \\
\cline{2-4}
& US-65 : Détection montants suspects & 4 & \begin{itemize}
...
\end{itemize} \\
\cline{2-4}
& US-66 : Analyse et extraction automatique facture & 4 & \begin{itemize}
...
\end{itemize} \\
\cline{2-4}
& US-67 : Comparaison accord/note & 8 & \begin{itemize}
 \item L’administrateur ou le manager peut consulter une analyse IA qui compare l’accord de mission avec la note.
...
\multirow{3}{*}{Dashboard} & US-69 : Dashboard employé & 5 & \begin{itemize}
...
\end{itemize} \\
\cline{2-4}
& US-70 : Dashboard manager & 8 & \begin{itemize}
...
\end{itemize} \\
\cline{2-4}
& US-71 : Dashboard administrateur & 8 & \begin{itemize}
...
Chatbot & US-72 : Chatbot hybride & 8 & \begin{itemize}
...
\includegraphics[width=1\textwidth,height=0.85\textheight]{authentification.png}
...
\begin{figure}[H]
 ...
 \includegraphics[width=1\textwidth,height=0.9\textheight]{seq_category.png}
...
\begin{itemize}
 \item[\huge$\star$] \textbf{\large Page d'accueil}
\end{itemize}
...
\end{figure}
\newpage
La figure ci-dessous montre la section "À propos" qui présente l'histoire et les valeurs de l'entreprise Coral-io.
...
\end{figure}
La figure ci-dessous montre la section "Expertises" qui illustre les domaines de compétence de Coral-io.
\begin{figure}[H]
...
\end{figure}
\newpage
La figure ci-dessous montre la section "Services" qui détaille l'offre proposée (DevOps, APM, Intégration, Finance et Assurance).
\begin{figure}[H]
...
La figure ci-dessous montre la section "Contact" avec le formulaire de contact et la FAQ.
...
\begin{itemize}
 \item[\huge$\star$] \textbf{\large Interface d'authentification}
\end{itemize}
...
\begin{itemize}
 \item[\huge$\star$] \textbf{\large Interface de récupération de mot de passe}
\end{itemize}
...
\begin{itemize}
 \item[\huge$\star$] \textbf{\large Interface de création/mise à jour du mot de passe}
\end{itemize}
...
\begin{itemize}
 \item[\huge$\star$] \textbf{\large Interface de mise à jour du profil utilisateur}
\end{itemize}
...
\includegraphics[width=\textwidth, height=0.3\textheight]{interface_update_profile.png}
...
\begin{itemize}
 \item[\huge$\star$] \textbf{\large Modal de confirmation de déconnexion}
\end{itemize}
...
\begin{itemize}
 \item[\huge$\star$] \textbf{\large Gestion des utilisateurs par l'administrateur}
\end{itemize}


\subsubsubsection{Interface de liste des utilisateurs}
...
\subsubsubsection{Interface d'ajout d'un utilisateur}
...
\subsubsubsection{Interface de modification d'un utilisateur}
...
\setcounter{subsubsubcounter}{0}

\begin{itemize}
 \item[\huge$\star$] \textbf{\large Gestion des profils}
\end{itemize}




\subsubsubsection{Interface de consultation du profil}
...
La figure ci-dessous montre l'interface de consultation du profil administrateur avec les informations personnelles et le statut du compte.
...
La figure ci-dessous montre la modale de changement de mot de passe.
...
\subsubsubsection{Interface de modification du profil}
...
La figure ci-dessous montre l'interface de modification du profil employé.
...
La figure ci-dessous montre l'interface de modification du profil manager.
...
\setcounter{subsubsubcounter}{0}
\begin{itemize}
 \item[\huge$\star$] \textbf{\large Gestion des départements}
\end{itemize}


\subsubsubsection{Interface de liste des départements}
...
\subsubsubsection{Interface d'ajout/modification d'un département}
...
\setcounter{subsubsubcounter}{0}
\begin{itemize}
 \item[\huge$\star$] \textbf{\large Gestion des projets}
\end{itemize}




\subsubsubsection{Interface de liste des projets}
...
\subsubsubsection{Interface d'ajout/modification d'un projet}
...
\subsubsubsection{Interface de consultation des projets (pour employé)}
...
\subsubsubsection{Interface de consultation des régles de remboursement}
...
\setcounter{subsubsubcounter}{0}
\begin{itemize}
 \item[\huge$\star$] \textbf{\large Gestion des catégories de dépenses}
\end{itemize}


\subsubsubsection{Interface de liste des catégories}
...
\subsubsubsection{Interface d'ajout/modification d'une catégorie avec champs \\ dynamiques}
...
La figure ci-dessous montre l'interface d'ajout/modification d'une catégorie avec le formulaire principal.
...
\newpage
La figure ci-dessous montre l'interface de gestion des champs des catégories pour ajouter des champs personnalisés.
\begin{figure}[H]
...
\setcounter{subsubsubcounter}{0}
\begin{itemize}
 \item[\huge$\star$] \textbf{\large Page « Ajouter une note de frais »}
\end{itemize}


\subsubsubsection{Interface d'ajout d'une note de frais}
...
\setcounter{subsubsubcounter}{0}
\begin{itemize}
 \item[\huge$\star$] \textbf{\large Page « Mes notes de frais »}
\end{itemize}


\subsubsubsection{Interface « Mes notes de frais »}
...
\subsubsubsection{Modal de confirmation de suppression d'une note}
...
\setcounter{subsubsubcounter}{0}
\begin{itemize}
 \item[\huge$\star$] \textbf{\large Page manager « Demandes à valider » }
\end{itemize}


\subsubsubsection{Interface de gestion des notes pour le manager}
...
\subsubsubsection{Interface de détails d'une note de frais}
...
\subsubsubsection{Interface de consultation de la consommation par employé}
...
\subsubsubsection{Modal de validation}
...
\subsubsubsection{Modal de refus}
...
\setcounter{subsubsubcounter}{0}
\begin{itemize}
 \item[\huge$\star$] \textbf{\large Liste déroulante des notifications}
\end{itemize}
...
La figure ci-dessous montre la liste déroulante des notifications pour un employé, accessible par l'icône de cloche dans l'en-tête.
...
La figure ci-dessous montre la liste déroulante des notifications pour un manager, avec les actions de validation et de refus.
...
La figure ci-dessous montre la liste déroulante des notifications pour un administrateur, avec les alertes de remboursement et de validation.
...
\subsubsubsection{Interface d'historique des notifications}
...
\begin{itemize}
 \item[\huge$\star$] \textbf{\large Liste des notes validées (page administrateur)}
\end{itemize}
...
\begin{itemize}
 \item[\huge$\star$] \textbf{\large Formulaire de remboursement (administrateur)}
\end{itemize}
...
\begin{itemize}
 \item[\huge$\star$] \textbf{\large Page des ordres de paiement en attente}
\end{itemize}
...
\begin{itemize}
 \item[\huge$\star$] \textbf{\large Page de détail d’un ordre de paiement}
\end{itemize}
...
\begin{itemize}
 \item[\huge$\star$] \textbf{\large Modal de confirmation de paiement}
\end{itemize}
...
\begin{itemize}
 \item[\huge$\star$] \textbf{\large Modal de confirmation de refus de remboursement}
\end{itemize}
...
\begin{itemize}
 \item[\huge$\star$] \textbf{\large Page d’archive administrative}
\end{itemize}
...
\begin{itemize}
 \item[\huge$\star$] \textbf{\large Notes internes (manager et administrateur)}
\end{itemize}
...
\begin{itemize}
 \item[\huge$\star$] \textbf{\large Dashboard projets administrateur}
\end{itemize}
...
\begin{itemize}
 \item[\huge$\star$] \textbf{\large Export Excel/CSV}
\end{itemize}
...
Pour simplifier le déploiement de l'application Coral.io Expense, nous avons conteneurisé chaque microservice ainsi que le serveur Keycloak et le serveur Ollama. Seule la base de données PostgreSQL n'est pas conteneurisée car elle sera utilisée via un service cloud en production. L'orchestration de l'ensemble est assurée par Docker Compose, permettant de lancer tous les services avec une seule commande.

\subsection{Définition et principes de la conteneurisation}

La conteneurisation est une technique de virtualisation légère qui permet d'empaqueter une application et ses dépendances dans un conteneur isolé. Contrairement à une machine virtuelle, un conteneur partage le noyau du système d'exploitation hôte, ce qui le rend plus rapide à démarrer et plus économe en ressources.

\textbf{Principes fondamentaux :}
\begin{itemize}
    \item \textbf{Isolation} : Chaque conteneur dispose de son propre système de fichiers, réseau et espaces de noms (namespaces).
    \item \textbf{Reproductibilité} : L'image Docker garantit le même environnement d'exécution partout (développement, test, production).
    \item \textbf{Légèreté} : Les conteneurs partagent le noyau hôte, réduisant l'empreinte mémoire et disque.
    \item \textbf{Orchestration} : Docker Compose permet de définir et gérer des applications multi-conteneurs via un fichier YAML.
\end{itemize}

\subsection{Architecture des images Docker : multi-stage builds}

Pour chaque microservice Java (expense-service, user-service, notification-service, etc.), nous utilisons un \texttt{Dockerfile} multi-stage. Cette approche présente plusieurs avantages : réduction de la taille finale de l'image, exclusion des outils de compilation, et meilleure sécurité.

\subsubsection{Exemple de Dockerfile multi-stage}

La figure ci-dessous présente le \texttt{Dockerfile} utilisé pour le \texttt{expense-service} (identique pour les autres services Spring Boot) :

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth,keepaspectratio]{dockerfile.png}
    \caption{Dockerfile multi-stage pour le expense-service}
    \label{fig:dockerfile_expense}
\end{figure}

\textbf{Explication des étapes :}
\begin{enumerate}
    \item \texttt{builder} : Étage de compilation contenant Maven et JDK. Il télécharge les dépendances et compile le code source.
    \item \texttt{run stage} : Image légère basée sur \texttt{temurin:21-jre-alpine} (environnement JRE uniquement, distribution Alpine Linux très compacte).
    \item Création d'un utilisateur non-root (\texttt{appuser}) pour des raisons de sécurité.
    \item Copie du JAR depuis l'étage \texttt{builder} vers l'image finale.
\end{enumerate}

\subsection{Orchestration avec Docker Compose}

Le fichier \texttt{docker-compose.yml} définit l'ensemble des services conteneurisés, leurs dépendances, variables d'environnement, réseaux et volumes. Nous avons structuré l'application en plusieurs services interconnectés.

\subsubsection{Configuration réseau}

La figure suivante montre la configuration réseau utilisée pour permettre la communication entre les conteneurs :

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth,keepaspectratio]{docker_network_config.png}
    \caption{Configuration réseau Docker Compose}
    \label{fig:docker_network}
\end{figure}

\subsubsection{Services clés et leurs rôles}

Le tableau suivant récapitule les services déployés :
...
\hline
Keycloak & 8090 & Gestion de l'authentification et des utilisateurs (SSO) \\
\hline
Config-server & 9999 & Configuration externalisée centralisée \\
\hline
Eureka-server & 8761 & Service discovery (enregistrement/localisation des microservices) \\
\hline
Gateway & 8888 & Point d'entrée unique, routage, CORS, sécurité \\
\hline
Expense-service & 8082 & Gestion des notes de frais \\
\hline
User-service & 8083 & Gestion des utilisateurs et profils \\
\hline
Notification-service & 8087 & Envoi d'emails (notifications de remboursement) \\
\hline
Chatbot-service & 8086 & Assistant conversationnel pour les employés \\
\hline
AI-microservice & 8085 & Analyse OCR, extraction de données, détection de doublons \\
\hline
Python-OCR-service & 9000 & Service Python pour l'analyse avancée (FAISS) \\
\hline
Frontend & 4200 & Interface Angular (Nginx) \\
\hline
Ollama & 11434 & Serveur de modèle LLM (Mistral/Gemma) pour l'IA \\
\hline
\end{tabular}
\caption{Liste des services Dockerisés}
\label{tab:services}
\end{table}

\subsubsection{Extrait du fichier docker-compose.yml}

La figure ci-dessous présente un extrait représentatif de la configuration docker-compose.yml :

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth,keepaspectratio]{docker_compose_extrait.png}
    \caption{Extrait du fichier docker-compose.yml}
    \label{fig:docker_compose_extrait}
\end{figure}

\subsubsection{Frontend : Dockerfile multi-stage pour Angular}

Le frontend Angular est également conteneurisé avec une approche multi-stage, comme illustré dans la figure suivante :

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth,keepaspectratio]{frontend_dockerfile.png}
    \caption{Dockerfile multi-stage pour le frontend Angular}
    \label{fig:frontend_dockerfile}
\end{figure}

Cette approche permet de servir l'application via Nginx, un serveur web performant et léger.

\subsection{Communication avec la base de données}

Puisque PostgreSQL n'est pas conteneurisé, les conteneurs y accèdent via une URL de connexion classique. Deux cas se présentent :

\begin{itemize}
    \item \textbf{En développement} : PostgreSQL tourne sur l'hôte (machine locale). Les conteneurs y accèdent via l'alias \texttt{host.docker.internal}.
    \item \textbf{En production} : PostgreSQL est utilisé via un service cloud (AWS RDS, Google Cloud SQL, etc.). Les conteneurs y accèdent via l'IP publique ou le nom de domaine du service cloud.
\end{itemize}

La figure ci-dessous montre la configuration de connexion à PostgreSQL dans le fichier \texttt{docker-compose.yml} :

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth,keepaspectratio]{clouddb_config.png}
    \caption{Configuration de connexion à PostgreSQL (développement et cloud)}
    \label{fig:clouddb_config}
\end{figure}

\textbf{Exemple de configuration :}
\begin{itemize}
    \item \texttt{DB\_host=host.docker.internal} → pour le développement local
    \item \texttt{DB\_host=monpostgres.cloud.com} → pour la production cloud
\end{itemize}

\subsection{Avantages}

\begin{itemize}
    \item \textbf{Environnement identique} : Élimine les problèmes de "ça marche sur ma machine".
    \item \textbf{Isolation} : Chaque service est isolé (réseau, processus, fichiers).
    \item \textbf{Déploiement simplifié} : Une seule commande \texttt{docker-compose up -d} lance toute l'application.
    \item \textbf{Persistance des données} : Les volumes Docker (\texttt{ai\_uploads}, \texttt{python\_ocr\_data}, \texttt{ollama\_data}) garantissent la conservation des données entre les redémarrages.
    \item \textbf{Passage à l'échelle} : Possibilité d'utiliser Docker Swarm ou Kubernetes pour orchestrer plusieurs instances.
    \item \textbf{Sécurité} : Utilisation d'utilisateurs non-root dans les images finales.
    \item \textbf{Base de données cloud} : PostgreSQL en cloud offre haute disponibilité, sauvegardes automatisées et mise à l'échelle sans gestion serveur.
\end{itemize}
\newpage
\subsection{Environnement d'exécution des conteneurs}

La figure ci-dessous montre l'état des conteneurs en cours d'exécution sur la machine de développement.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth,keepaspectratio]{docker_interface.png}
    \caption{Conteneurs Docker en cours d'exécution (vue Docker Desktop)}
    \label{fig:docker_containers}
\end{figure}

\subsection{Transition vers le cloud}

Pour une utilisation en production, plusieurs options s'offrent à nous :

\begin{itemize}
    \item \textbf{Base de données} : Utilisation d'un service managé PostgreSQL (AWS RDS, Google Cloud SQL, Azure Database)
    \item \textbf{Ollama} : Déploiement sur une instance GPU cloud ou utilisation d'une API LLM externe
    \item \textbf{Microservices} : Déploiement sur Kubernetes (GKE, EKS, AKS) ou sur des instances EC2 avec Docker Compose
\end{itemize}

\subsection{Conclusion du déploiement}

L'architecture conteneurisée permet un déploiement fiable et reproductible de Coral.io Expense. Les choix techniques (multi-stage builds, réseau bridge, volumes, utilisateur non-root) garantissent une application prête pour la production. L'utilisation d'une base de données cloud pour PostgreSQL assure une gestion professionnelle des données, tandis que la conteneurisation d'Ollama permet une intégration flexible de l'IA, pouvant être facilement remplacée par un service cloud si nécessaire.
...
\captionof{table}{Backlog sprint 4}
...
\subsection{Diagramme des cas d'utilisation global du sprint 4}
...
\subsection*{Description textuelle -- Dépôt et analyse d'un justificatif}

\captionof{table}{Description textuelle – Dépôt et analyse d'un justificatif}
...
& 9. Si un accord de mission est associé à la note, le système compare sémantiquement les données via LLM (Gemma3:4b) \\
...
\includegraphics[width=0.9\textwidth,height=0.8\textheight]{class_diagram_sprint4.png}
...
\subsection{Application de CRISP-DM}

La détection des anomalies dans les notes de frais ne se limite pas à l’implémentation d’un modèle : elle nécessite une approche structurée couvrant l’ensemble du cycle de vie des données. Nous avons donc appliqué la méthodologie CRISP-DM selon les phases suivantes :

\begin{itemize}
 \item \textbf{Compréhension du métier} : Définition des anomalies à détecter (doublons, montants suspects, incohérences accord/note) en collaboration avec les managers et administrateurs.
 
 \item \textbf{Compréhension des données} : Analyse des justificatifs (factures, accords) et de l'historique des notes de frais.
 
 \item \textbf{Préparation des données} : Nettoyage des textes OCRisés, indexation FAISS, construction des historiques de montants.
 
 \item \textbf{Modélisation} : Test et sélection des modèles (FAISS + SentenceTransformer pour les doublons, Isolation Forest pour les montants suspects, LLM Gemma3:4b pour l'extraction).
 
 \item \textbf{Évaluation} : Calcul des métriques (précision, rappel, F1-score) et ajustement des seuils.
 
 \item \textbf{Déploiement} : Intégration des modèles dans les microservices Python et affichage des résultats dans l'interface web.
\end{itemize}
...
\subsubsection{Compréhension du métier}
...
\item Analyser les factures (OCR + LLM) pour vérifier la validité du justificatif et en extraire automatiquement les informations clés (dates, montants, fournisseur, TVA), permettant ainsi un pré-remplissage du formulaire de note de frais.
\item Analyser l'accord de mission signé via un couplage entre règles prédéfinies (vérification formelle) et LLM (analyse sémantique), permettant de détecter automatiquement les incohérences contextuelles.
...
\subsubsection{Compréhension des données}
...
\subsubsubsub{Collecte des données}
...
\subsubsubsection{ Sources des données}
...
\subsubsubsection{Présentation et évaluation des outils OCR}\\
...
\subsubsubsection{Processus de collecte}\\
...
\end{itemize}
\newpage
\subsubsubsection{ Exemples d’extraction par PaddleOCR et Tesseract}\\
...
\begin{itemize}
 ...
 \includegraphics[width=0.9\textwidth,height=0.8\textheight]{paddle_ocr.png}
...
\newline
 ...
 \includegraphics[width=0.7\textwidth,height=0.7\textheight]{paddle_ocr2.png}
...
\newpage
 ...
 \includegraphics[width=0.9\textwidth,height=0.8\textheight]{tesseract.png}
...
\subsubsubsub{Exploration des données}
...
\subsubsubsection{Chargement des données}
...
\subsubsubsection{Nettoyage et normalisation des données}
...
\begin{figure}[H]
 ...
 \includegraphics[width=0.8\textwidth]{text_cleaner1.png}
...
\subsubsubsection{Indexation FAISS pour la détection de doublons}
...
\begin{figure}[H]
 ...
 \includegraphics[width=0.8\textwidth]{add_to_index1.png}
...
\subsubsubsection{Construction de l'historique des montants}
...
\begin{figure}[H]
 ...
 \includegraphics[width=0.7\textwidth]{get_user_category1.png}
...
\subsubsubsection{Extraction structurée des accords de mission}
...
\subsubsubsection{Alignement des données pour la comparaison}
...
\begin{figure}[H]
 ...
 \includegraphics[width=0.85\textwidth]{compare1.png}
...
\subsubsubsection{Détection des doublons de justificatifs}
...
\centering
\fbox{\includegraphics[width=0.6\textwidth]{logo_hash.png}}  % ← Réduit de 0.8 à 0.6
...
\begin{minipage}{0.3\textwidth}
\centering
\fbox{\includegraphics[width=0.6\textwidth]{logo_bert.png}}  % ← Réduit de 0.8 à 0.6
...
\begin{minipage}{0.3\textwidth}
\centering
\fbox{\includegraphics[width=0.6\textwidth]{logo_faiss.png}}  % ← Réduit de 0.8 à 0.6
\captionof*{figure}{Logo SentenceTransformer}
\end{minipage}
\hfill
\begin{minipage}{0.65\textwidth}
\textbf{FAISS + SentenceTransformer (all-MiniLM-L6-v2)} : Notre approche retenue. Le texte est transformé en embedding de 384 dimensions via SentenceTransformer, puis indexé dans FAISS pour une recherche rapide.
\end{minipage}

\newpage
La figure suivante illustre un extrait de code où cette approche a été implémentée pour la détection des doublons.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{detect_duplicate_code.png}
\caption{Implémentation de FAISS + SentenceTransformer}
\label{fig:faiss_code}
\end{figure}

\subsubsubsection{ Remplissage automatique des champs du formulaire}
...
\begin{minipage}{0.3\textwidth}
\centering
\fbox{\includegraphics[width=0.8\textwidth]{logo_mistral.png}}
...
\begin{minipage}{0.3\textwidth}
\centering
\fbox{\includegraphics[width=0.8\textwidth]{logo_llama.png}}
...
\begin{minipage}{0.3\textwidth}
\centering
\fbox{\includegraphics[width=0.8\textwidth]{logo_gemma.png}}
...
\vspace{0.5cm}
La figure suivante illustre l'implémentation de l'appel au modèle Gemma3:4b via l'API d'Ollama pour l'extraction des données structurées à partir des documents scannés.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth,height=0.5\textheight]{gemma_code.png}
\caption{Utilisation de Gemma3:4b }
\label{fig:gemma_code}
\end{figure}

\subsubsubsection{Détection des montants suspects (Isolation Forest)}

Plusieurs approches ont été testées pour la détection des montants suspects (Z-score, DBSCAN, One-Class SVM, Isolation Forest). La figure suivante illustre l'implémentation de l'Isolation Forest pour la détection des montants suspects, combinée à une vérification statistique par écart-type ($2\sigma$) pour capturer les valeurs extrêmes.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{detect_anomaly_code.png}
\caption{Implémentation de l'Isolation Forest}
\label{fig:isolation_forest_code}
\end{figure}
\subsubsubsection{ Comparaison accord/note – Règles métier + LLM (Gemma3:4b)}

Plusieurs modèles ont été testés pour la comparaison accord/note (règles métier seules, LLM seul, approche hybride). L'approche retenue combine des règles métier déterministes pour les vérifications objectives (identité, dates, projet, localisation) et un LLM (Gemma3:4b) pour l'analyse sémantique fine. Les figures suivantes présentent les implémentations des deux composantes.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{accord_comparator_code.png}
\caption{Implémentation des règles métier pour la comparaison accord/note}
\label{fig:accord_comparator_code}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{ia_form_analyzer_code.png}
\caption{Implémentation de l'analyse LLM pour la comparaison accord/note}
\label{fig:ia_form_analyzer_code}
\end{figure}
\subsubsubsection{ Analyse de l'accord de mission}

Plusieurs méthodes d'extraction ont été testées pour l'analyse des accords de mission (regex seules, OCR seul, OCR + LLM). L'approche retenue combine une extraction par regex pour les champs structurés, avec un fallback LLM (Gemma3:4b) lorsque les regex échouent. La figure suivante illustre l'implémentation complète de l'analyse d'accord via la classe \texttt{AuditService}.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{audit_service_code.png}
\caption{Implémentation de l'analyse complète de l'accord de mission}
\label{fig:audit_service_code}
\end{figure}
...
\subsubsection{Résultats et décisions de sélection des modèles}

Cette section présente les résultats obtenus lors de l'évaluation des différents modèles testés. Sur la base de ces résultats, nous avons justifié nos choix finaux.

\subsubsubsection{Évaluation de la détection des doublons}
...
\centering
\includegraphics[width=\linewidth,height=0.25\textheight]{cm_hash.png}
\caption{Matrice de confusion - Hash MD5}
\label{fig:cm_hash}
\end{minipage}
\hfill
\begin{minipage}{0.45\textwidth}
\centering
\includegraphics[width=\linewidth,height=0.25\textheight]{cm_bert.png}
\caption{Matrice de confusion - BERT}
\label{fig:cm_bert}
\end{minipage}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.5\linewidth,height=0.25\textheight]{cm_faiss.png}
\caption{Matrice de confusion - FAISS + SentenceTransformer}
\label{fig:cm_faiss}
\end{figure}

Le tableau suivant compare les performances des trois modèles testés.
...
\hline
FAISS + SentenceTransformer & 0,98 & 0,96 & 0,97 & 120 \\
\hline
\end{tabular}
\end{table}

La figure suivante visualise ces écarts de performance.

\begin{figure}[H]
\centering
\includegraphics[width=0.7\linewidth]{f1_score_doublons.png}
\caption{Comparaison des F1-scores pour la détection des doublons}
\label{fig:f1_duplicate}
\end{figure}

\subsubsubsection{Évaluation de l'extraction des champs par LLM}
...
\begin{minipage}{0.45\textwidth}
\centering
\includegraphics[width=\linewidth,height=0.25\textheight]{cm_mistral.png}
\caption{Matrice de confusion - Mistral 7B}
\label{fig:cm_mistral}
\end{minipage}
\hfill
\begin{minipage}{0.45\textwidth}
\centering
\includegraphics[width=\linewidth,height=0.25\textheight]{cm_gemma.png}
\caption{Matrice de confusion - Gemma3:4b}
\label{fig:cm_gemma}
\end{minipage}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.5\linewidth,height=0.25\textheight]{cm_llama.png}
\caption{Matrice de confusion - Llama 3 8B}
\label{fig:cm_llama}
\end{figure}

Le tableau suivant compare les performances des trois LLM testés pour l'extraction automatique des champs.
...
\hline
Gemma3:4b & 0,95 & 0,93 & 0,94 & 450 \\
\hline
\end{tabular}
\end{table}

La figure suivante confirme visuellement la supériorité de Gemma3:4b sur l'ensemble des critères.

\begin{figure}[H]
\centering
\includegraphics[width=0.7\linewidth]{radar_llm.png}
\caption{Comparaison multicritères des modèles LLM}
\label{fig:radar_llm}
\end{figure}

\subsubsubsection{Décisions de sélection des modèles}

En nous basant sur les résultats présentés ci-dessus, nous avons arrêté les choix suivants :

\begin{itemize}
\item \textbf{Détection des doublons} : Nous avons retenu \textbf{FAISS + SentenceTransformer (all-MiniLM-L6-v2)} car il offre le meilleur compromis entre précision (98\%), rappel (96\%) et rapidité (120 ms/fichier). L'index persistant permet une mise à jour incrémentale.

\item \textbf{Remplissage automatique du formulaire} : Nous avons choisi \textbf{Gemma3:4b} pour sa rapidité (450 ms) et son F1-score élevé (0,94), supérieur à Mistral 7B (0,89) et Llama 3 8B (0,92).

\item \textbf{Détection des montants suspects} : Nous avons conservé \textbf{l'Isolation Forest} après avoir testé plusieurs approches (Z-score, DBSCAN, One-Class SVM). Cette méthode est combinée à une vérification statistique par écart-type ($2\sigma$) pour capturer les valeurs extrêmes.

\item \textbf{Analyse de l'accord de mission} : Nous avons opté pour l' \textbf{approche hybride regex + LLM (Gemma3:4b)}. L'extraction par regex assure une rapidité optimale pour les champs structurés, tandis que le LLM sert de fallback intelligent lorsque les regex échouent ou que des champs sont manquants.

\item \textbf{Comparaison accord/note} : Nous avons opté pour une \textbf{combinaison règles métier + LLM (Gemma3:4b)}. Cette approche hybride permet de bénéficier de la robustesse des règles métier pour les vérifications objectives (identité, dates, projet, localisation) et de la flexibilité du LLM pour l'analyse sémantique fine des incohérences.
\end{itemize}
...
\begin{figure}[H]
 ...
 \includegraphics[width=1\textwidth]{code_sauvegarde_faiss1.png}
...
La figure ci-dessous montre le formulaire de création d'une note de frais avant l'extraction automatique : tous les champs (montant, date, description, commerçant) sont vides et doivent être saisis manuellement par l'employé.

\begin{figure}[H]
 \centering
 \includegraphics[width=\textwidth]{auto_fill_fields_avant.png}
 \caption{Formulaire de note de frais avant remplissage automatique}
 \label{fig:auto_fill_fields_avant}
\end{figure}

La figure ci-dessous montre le même formulaire après l'analyse automatique de la facture téléchargée : les champs montant, date, description et commerçant ont été automatiquement extraits et pré-remplis par le système, permettant à l'employé de gagner du temps et d'éviter les erreurs de saisie.

\begin{figure}[H]
 \centering
 \includegraphics[width=\textwidth]{auto_fill_fields_aprés.png}
 \caption{Formulaire de note de frais après remplissage automatique}
 \label{fig:auto_fill_fields_apres}
\end{figure}
...
L’administrateur ou le manager peut consulter l'analyse IA qui compare l’accord de mission (PDF signé) avec les données du formulaire de la note. Les résultats sont présentés dans un bloc dédié au sein du popup de détail, organisé en trois onglets :
...
\begin{itemize}
 ...
 \includegraphics[width=1\textwidth]{chatbot/chatbot_patterns1.png}
...
\begin{figure}[H]
 ...
 \includegraphics[width=1\textwidth]{chatbot/chatbot_prompt1.png}
...
\begin{figure}[H]
 ...
 \includegraphics[width=1\textwidth]{chatbot/chatbot_session1.png}
...
\begin{figure}[H]
 ...
 \includegraphics[width=1\textwidth]{chatbot/chatbot_routing1.png}
...
L'application de CRISP-DM a permis de déployer des modèles d'IA (FAISS, Isolation Forest, LLM) couvrant les principaux cas de fraude. Les tableaux de bord interactifs facilitent la décision, tandis que le chatbot hybride améliore l'expérience utilisateur. Ces travaux posent une base solide pour l'amélioration continue.
